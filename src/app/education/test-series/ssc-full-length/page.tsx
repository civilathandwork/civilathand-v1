      ...
      <main className="min-h-screen bg-slate-100">
        <div style={{ background: "linear-gradient(135deg, #4a148c 0%, #6a1b9a 100%)" }}>
          <div className="max-w-5xl mx-auto px-4 py-10">
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-white mb-2">SSC-JE — Full Length Tests</h1>
            <p className="text-white/75 text-sm max-w-xl leading-relaxed">
              Built to the SSC-JE Paper-1 pattern (General Intelligence &amp; Reasoning, General
              Awareness and Civil &amp; Structural). Real exam interface with timer and solutions.
            </p>
          </div>
        </div>
        ...
        <div className="max-w-5xl mx-auto px-4 py-8 grid gap-4 sm:grid-cols-2">
          {tests.map((t) => (
            <div key={t.id} className="bg-white rounded-xl border border-slate-200 p-5 flex flex-col hover:border-orange-400 hover:shadow-md transition-all">
              ...
              <Link href={`/education/test-series/ssc-full-length/${t.id}`}
